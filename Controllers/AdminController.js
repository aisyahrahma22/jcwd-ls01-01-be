...
module.exports = {
  ...
  searchKartuStok: async (req, res) => {
    try {
      ...
      if (data || bulan || tahun) {
        db.query(
          `SELECT detailstokproduk.*, admin.username FROM detailstokproduk 
      JOIN admin ON detailstokproduk.admin_id=admin.id 
      WHERE MONTH(detailstokproduk.created_at) = ${bulan} AND YEAR(detailstokproduk.created_at) = ${tahun} AND detailstokproduk.Produk_id = ${produkId} AND detailstokproduk.aktivitas = "${data}"`,
          (err, result) => {
            try {
              if (err) throw err;
  ...
  transactionCount: async (req, res) => {
    try {
      ...
      if (status === 'resep-baru') {
        query1 += `WHERE statusTransaksi_id = 1 `;
      }
      if (status === 'menunggu-bukti-pembayaran') {
        query1 += `WHERE statusTransaksi_id = 2 `;
      }
      if (status === 'cek-bukti-pembayaran') {
        query1 += `WHERE statusTransaksi_id = 3 `;
      }
      if (status === 'pesanan-diproses') {
        query1 += `WHERE statusTransaksi_id = 4 `;
      }
      if (status === 'dalam-pengiriman') {
        query1 += `WHERE statusTransaksi_id = 5 `;
      }
      if (status === 'pesanan-selesai') {
        query1 += `WHERE statusTransaksi_id = 6 `;
      }
      if (status === 'pesanan-dibatalkan') {
        query1 += `WHERE statusTransaksi_id = 7 `;
      }
      if (status === 'menunggu-checkout') {
        query1 += `WHERE statusTransaksi_id = 8 `;
      }
  ...
  getBukuKas: async (req, res) => {
    try {
      // GET DATA tanggal, aktifitas, keluar
      let query1 = `SELECT id, no_pemesanan as noFakturKeluar, total_pembayaran as hargaKeluar, created_at as tanggalKeluar FROM transaksi WHERE statusTransaksi_id = 6 ORDER BY id;`;
      const data = await query(query1);
      console.log('data', data);

      // GET DATA tanggal, aktifitas, masuk
      let query2 = `SELECT detailstokproduk.id as noFakturMasuk, detailstokproduk.masuk as stokMasuk , detailstokproduk.created_at as tanggalMasuk, detailstokproduk.aktivitas, produk.nilai_barang as hargaBeli FROM detailstokproduk 
